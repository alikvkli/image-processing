class ImageProcessor {
    ...
    private blurMult: Int32Array[] = [];

    private getARGB(data: Uint8ClampedArray, i: number): number {
        const offset = i * 4;
        return (
            ((data[offset + 3] << 24) & 0xff000000) |
            ((data[offset] << 16) & 0x00ff0000) |
            ((data[offset + 1] << 8) & 0x0000ff00) |
            ((data[offset + 2] & 0x0000ff00))
        );
    }
    ...
    private blurARGB(pixels: Uint8ClampedArray, canvas: HTMLCanvasElement, radius: number) {
    ...
    private invertColors(pixels: Uint8ClampedArray) {
    ...
    private dilate(pixels: Uint8ClampedArray, canvas: HTMLCanvasElement) {
        let currIdx = 0;
    ...
    preprocessImage(canvas: HTMLCanvasElement) {
        ...
        this.blurARGB(image.data, canvas, 1);
        this.dilate(image.data, canvas);
        this.invertColors(image.data);
        this.thresholdFilter(image.data, 0.4);
        return image;
    }
    ...
    processImage(canvas: HTMLCanvasElement, thresholdLevel: number) {
        const image = canvas.getContext('2d')!.getImageData(0, 0, canvas.width, canvas.height);
        this.thresholdFilter(image.data, thresholdLevel);
        return image;
    }
}
